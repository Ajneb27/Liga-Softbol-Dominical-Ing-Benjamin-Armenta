...
# --- 1. CONFIGURACIÓN DE LIGA (Agosto 2024 - Febrero 2026) ---
...
def cargar_equipos():
    ...
    if os.path.exists(EQUIPOS_FILE):
        try:
            ...
            return df
        except: return pd.DataFrame(columns=cols_e)
    return pd.DataFrame(columns=cols_e)
...
if menu == "🏆 LÍDERES":
    ...
    with t1:
        c1, c2 = st.columns(2)
        with c1:
            st.write("**HITS (H)**"); st.table(df_j.nlargest(10, 'H', keep='all')[['Nombre', 'H']])
            st.write("**DOBLES (2B)**"); st.table(df_j.nlargest(10, '2B', keep='all')[['Nombre', '2B']])
        with c2:
            st.write("**HOME RUNS (HR)**"); st.table(df_j.nlargest(10, 'HR', keep='all')[['Nombre', 'HR']])
            st.write("**TRIPLES (3B)**"); st.table(df_j.nlargest(10, '3B', keep='all')[['Nombre', '3B']])
    with t2:
        st.subheader("Líderes de Pitcheo")
        cp1, cp2 = st.columns(2)
        with cp1:
            st.write("**GANADOS (G)**"); st.table(df_j.sort_values('G', ascending=False).head(10)[['Nombre', 'G']])
        with cp2:
            st.write("**PERDIDOS (P)**"); st.table(df_j.sort_values('P', ascending=False).head(10)[['Nombre', 'P']])

# --- 6. SECCIÓN EQUIPOS (RESTAURADA CON TRAYECTORIA) ---
elif menu == "🏘️ EQUIPOS":
    st.header("🏘️ Gestión de Equipos y Trayectoria")
    if st.session_state.admin:
        with st.form("eq_f", clear_on_submit=True):
            n=st.text_input("Nombre del Equipo:"); d=st.number_input("Año Debut:", 2024, 2026, 2024); f=st.number_input("Año Retiro (0 si activo):", 0, 2026, 0)
    ...
    if not df_e.empty:
        df_v = df_e.copy()
        # Cálculo: (Año Fin o Actual) - Debut + 1
        ...
        st.subheader("Lista de Equipos Ordenados por Antigüedad")
        st.table(df_v.sort_values("Temporadas", ascending=False)[["Nombre", "Debut", "Estatus", "Temporadas"]])

# --- 7. REGISTRAR (AUTOCLEAN Y AUTOCOMPLETADO) ---
