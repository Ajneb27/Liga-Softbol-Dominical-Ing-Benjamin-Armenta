...
if menu == "🏠 INICIO":
    ...
    c1, c2 = st.columns(2)
    min_vb = c1.number_input("Min VB:", 0, 500, int(jor_max * 2.5), disabled=not st.session_state.admin)
    min_ip = c2.number_input("Min IP:", 0, 500, int(jor_max * 2), disabled=not st.session_state.admin)
    ...
    if not df_j.empty:
        ...
        hits_t = d['H'] + d['2B'] + d['3B'] + d['HR']
        avg = hits_t / d['VB'] if d['VB'] > 0 else 0
        
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Equipo", d['Equipo']); c2.metric("VB", int(d['VB'])); c3.metric("TB", int(d['VB']+d['BB'])); c4.metric("AVG", f"{avg:.3f}")
        
        col_a, col_b = st.columns(2)
        with col_a:
            st.write("### ⚾ Bateo")
            st.write(f"**Hits:** {int(d['H'])} | **2B:** {int(d['2B'])} | **3B:** {int(d['3B'])} | **HR:** {int(d['HR'])} | **BB:** {int(d['BB'])}")
            st.bar_chart(pd.DataFrame({"Cant": [d['H'], d['2B'], d['3B'], d['HR']]}, index=["H", "2B", "3B", "HR"]))
        with col_b:
            st.write("### 🎯 Pitcheo")
            st.write(f"**G:** {int(d['G'])} | **P:** {int(d['P'])} | **JI:** {int(d['JI'])} | **IP:** {d['IP']}")

elif menu == "🏘️ EQUIPOS":
    st.header("🏘️ Gestión de Equipos")
    if st.session_state.admin:
        tl, te = st.tabs(["Nuevo", "Editar"])
        with tl:
            with st.form("n_eq"):
    ...
    with st.expander("🧨 RESET TEMPORADA"):
        clave = st.text_input("Clave (RESET2026):")
