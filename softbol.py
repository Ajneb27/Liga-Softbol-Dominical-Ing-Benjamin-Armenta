...
if menu == "🏠 Inicio":
    ...
    if fotos:
        ...
        for i, f in enumerate(fotos[:3]):
            with c_gal[i]: st.image(os.path.join(CARPETA_FOTOS, f), use_container_width=True)
    st.divider()
...
# ==========================================
    ...
    with t2:
        ...
        if not dfp.empty:
            ...
            cp1.subheader("🥇 Efectividad"); cp1.table(dfp[dfp['IP']>0].sort_values("EFE").head(10)[["Nombre", "EFE"]].style.format({"EFE": "{:.2f}"}))
