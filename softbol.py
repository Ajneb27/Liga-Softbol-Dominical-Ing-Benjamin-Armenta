...
# --- 3. BARRA LATERAL (SEGURIDAD ADMIN) ---
with st.sidebar:
    ...
    if st.session_state.admin:
        opciones += ["✍️ REGISTRAR", "🗑️ BORRAR", "💾 RESPALDO"]
...
if menu == "🏠 INICIO":
    ...
    # Turnos mínimos configurados por Admin o por defecto para visitantes
    ...
    if not df_e.empty:
        ...
        if not df_r.empty:
            ...
            st.dataframe(df_r[["Nombre", "TB", "VB", "H", "2B", "3B", "HR", "BB", "G", "P", "JI", "AVG"]], hide_index=True, use_container_width=True)
    ...
    if not df_j.empty:
        ...
        c2.metric("VB", int(d['VB']))
        c3.metric("JI (Iniciados)", int(d['JI']))
        c4.metric("AVG", f"{((d['H']+d['2B']+d['3B']+d['HR'])/d['VB'] if d['VB']>0 else 0):.3f}")
        st.write(f"**Bateo:** TB (Apariciones): {int(d['VB']+d['BB'])} | H: {int(d['H'])} | 2B: {int(d['2B'])} | 3B: {int(d['3B'])} | HR: {int(d['HR'])} | BB: {int(d['BB'])}")
    ...
    if st.session_state.admin:
        with st.form("nuevo_eq"):
            ...
            if st.form_submit_button("Añadir Equipo") and n_eq:
                df_e = pd.concat([df_e, pd.DataFrame([{"Nombre":n_eq,"Debut":ANIO_ACTUAL,"Logo":LOGO_DEFECTO}])], ignore_index=True)
    ...
    with t_j:
        with st.form("rj"):
            ...
            eq = st.selectbox("Equipo", df_e["Nombre"].unique()) if not df_e.empty else ""
            c1, c2, c3 = st.columns(3)
            ...
            if st.form_submit_button("Guardar"):
                if n in df_j["Nombre"].values:
                    ...
                    df_j = pd.concat([df_j, pd.DataFrame([{"Nombre":n,"Equipo":eq,"VB":vb,"H":h,"2B":h2,"3B":h3,"HR":hr,"BB":bb,"G":pg,"P":pp,"JI":ji}])], ignore_index=True)
    ...
    st.header("💾 Respaldos")
    st.download_button("📥 Descargar Jugadores (CSV)", df_j.to_csv(index=False), "jugadores_2026.csv")
    st.download_button("📥 Descargar Resultados (CSV)", df_g.to_csv(index=False), "resultados_2026.csv")
