...
# --- 🖼️ SECCIÓN GALERÍA (PARA AGREGAR FOTOS) ---
elif menu == "🖼️ Galería":
    st.title("📸 Galería de la Liga")
    if st.session_state.rol == "Admin":
        with st.expander("⬆️ SUBIR NUEVA FOTO (SOLO ADMIN)"):
            archivo = st.file_uploader("Selecciona imagen:", type=['png', 'jpg', 'jpeg'])
            if st.button("Guardar Imagen") and archivo:
                with open(os.path.join(FOTOS_DIR, archivo.name), "wb") as f:
                    f.write(archivo.getbuffer())
                st.success("Foto subida!"); st.rerun()

    fotos = [f for f in os.listdir(FOTOS_DIR) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    if fotos:
        cols = st.columns(3)
        for i, f in enumerate(fotos):
            with cols[i % 3]: st.image(os.path.join(FOTOS_DIR, f), use_container_width=True)
    else: st.info("No hay fotos aún.")
...
# --- STANDINGS Y ROSTERS (CALCULADOS) ---
elif menu == "📊 Standings":
    st.title("📊 Posiciones")
