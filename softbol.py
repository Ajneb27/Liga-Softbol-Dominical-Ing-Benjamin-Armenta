...
if menu == "🏠 INICIO":
    ...
    if not df_j.empty:
        ...
        c1, c2 = st.columns(2)
        with c1: st.info(f"**BATEO:** VB: {int(d['VB'])} | H: {int(d['H'])} | 2B: {int(d['2B'])} | 3B: {int(d['3B'])} | HR: {int(d['HR'])}")
        with c2: st.success(f"**PITCHEO:** Juegos Ganados: {int(d['G'])} | Juegos Perdidos: {int(d['P'])}")
    ...
    csv_data = df_j.to_csv(index=False)
    st.download_button("📥 Descargar CSV", csv_data, "respaldo_jugadores.csv", "text/csv")
    st.divider()
    f = st.file_uploader("Subir CSV para restaurar", type="csv")
    if f: 
        df_upload = pd.read_csv(f)
        df_upload.to_csv(JUGADORES_FILE, index=False)
