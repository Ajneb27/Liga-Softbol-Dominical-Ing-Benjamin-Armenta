...
# Reemplaza esta URL con el link directo de tu logo
...
if menu == "🏠 INICIO":
    ...
    with c_inf:
        ...
        st.info("¡Bienvenido al portal oficial de estadísticas! Navega en el menú lateral para ver rosters y líderes.")
    with c_cal:
        ...
        else: st.info("No hay juegos programados en el calendario.")
    ...
    else: st.info("Registra resultados en el panel de administrador para ver el standing.")
    ...
    with t2:
        ...
        if not df_p.empty:
            c1, c2 = st.columns(2)
            c1.subheader("Juegos Ganados (G)"); c1.dataframe(df_p.nlargest(10, 'G')[['Nombre', 'Equipo', 'G']], hide_index=True)
            c2.subheader("Juegos Perdidos (P)"); c2.dataframe(df_p.nlargest(10, 'P')[['Nombre', 'Equipo', 'P']], hide_index=True)
        else: st.info("Solo se muestran jugadores con actividad en el pitcheo.")
    ...
    if not df_e.empty:
        ...
        st.dataframe(df_roster[["Nombre", "Categoria", "VB", "H", "2B", "3B", "HR", "AVG"]].sort_values("AVG", ascending=False), use_container_width=True, hide_index=True)
        ...
        with tr:
            with st.form("fr"):
                ...
                if st.form_submit_button("💾 GUARDAR SCORE"):
                    pd.concat([df_games, pd.DataFrame([{"Jornada":jor, "Visitante":v, "CarrerasV":cv, "HomeClub":h, "CarrerasH":ch}])], ignore_index=True).to_csv(JUEGOS_FILE, index=False); st.success("Resultado guardado."); st.rerun()
        with tc:
            with st.form("fc"):
                ...
                if st.form_submit_button("📅 PROGRAMAR"):
                    pd.concat([df_cal, pd.DataFrame([{"Jornada":jor_c, "Fecha":str(f), "Hora":"10:00 AM", "Visitante":v_c, "HomeClub":h_c, "Campo":cam}])], ignore_index=True).to_csv(CALENDARIO_FILE, index=False); st.success("Programado!"); st.rerun()
    ...
    if st.session_state.admin:
        j_d = st.selectbox("Eliminar Jugador Permanentemente:", [""] + sorted(df_j["Nombre"].tolist()))
        if st.button("❌ ELIMINAR"):
            df_j[df_j["Nombre"] != j_d].to_csv(JUGADORES_FILE, index=False); st.success("Eliminado"); st.rerun()
    else: st.warning("Área restringida.")

elif menu == "💾 RESPALDO":
    if st.session_state.admin:
        st.subheader("💾 Gestión de Copias de Seguridad")
        st.info("Descarga tus datos después de cada jornada para evitar pérdidas.")
        ...
        st.error("⛔ Acceso denegado. Solo el administrador puede descargar respaldos.")
