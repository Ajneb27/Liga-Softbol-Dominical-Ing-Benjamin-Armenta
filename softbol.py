...
# --- 2. CARGA DE DATOS REAL (SIN CACHÉ) ---
...
if menu == "🏃 Admin General":
    if st.session_state.rol != "Admin":
        ...
        st.warning("👉 **REGLA:** Edita/Borra -> Presiona ENTER -> Presiona GUARDAR.")

        t1, t2, t3, t4 = st.tabs(["Equipos", "Bateadores", "Pitchers", "Calendario"])

        with t1:
            df_e = cargar_csv("data_equipos.csv", ["Nombre"])
        ...
        with t2:
            df_j = cargar_csv("data_jugadores.csv", ["Nombre","Equipo","VB","H","H2","H3","HR"])
        ...
        with t3:
            df_p = cargar_csv("data_pitchers.csv", ["Nombre","Equipo","JG","JP","IP","CL","K"])
        ...
        with t4:
            df_c = cargar_csv("data_calendario.csv", ["Jornada","Fecha","Hora","Campo","Local","Visitante","Score"])
...
# --- 5. ROSTERS (FILTRO SEGURO) ---
    ...
    if not df_e.empty:
        # Limpieza de nombres para evitar que el filtro falle
        df_j["Equipo"] = df_j["Equipo"].astype(str).str.strip().str.upper()
        df_p["Equipo"] = df_p["Equipo"].astype(str).str.strip().str.upper()
        
        lista_e = sorted(df_e["Nombre"].astype(str).str.strip().unique().tolist())
        eq_sel = st.selectbox("Selecciona Equipo:", lista_e)
        
        c1, c2 = st.columns(2)
        with c1:
            st.subheader("🥖 Bateadores")
            db = df_j[df_j["Equipo"] == eq_sel.upper()].copy()
            if not db.empty:
        ...
        with c2:
            ...
            dp = df_p[df_p["Equipo"] == eq_sel.upper()].copy()
            if not dp.empty:
