...
# Estilo CSS para Tablas Doradas
st.markdown("""
    <style>
    .main { background-color: #f5f7f9; }
    .stMetric { background-color: #ffffff; border-left: 5px solid #d4af37; padding: 10px; border-radius: 5px; }
    .gold-header { color: #d4af37; font-weight: bold; border-bottom: 2px solid #d4af37; padding-bottom: 5px; }
    </style>
...
# --- 6. SECCIÓN ✍️ REGISTRAR (CALCULADORA INTELIGENTE) ---
    ...
    if st.session_state.admin:
        ...
        st.subheader("Acciones del día:")
        c1, c2, c3, c4 = st.columns(4)
        if c1.button("Sencillo (+1H)"): st.session_state.vals["H"]+=1; st.session_state.vals["VB"]+=1; st.rerun()
        if c2.button("Home Run (+1HR)"): st.session_state.vals["H"]+=1; st.session_state.vals["HR"]+=1; st.session_state.vals["VB"]+=1; st.rerun()
        if c3.button("Ganó Juego (+1G)"): st.session_state.vals["G"]+=1; st.rerun()
        if c4.button("Perdió Juego (+1P)"): st.session_state.vals["P"]+=1; st.rerun()
        ...
        with st.form("f_reg", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 GUARDAR"):
                ...
                nueva = pd.DataFrame([{"Nombre": nom_f, "Equipo": eq_f, "VB": vb, "H": h, "2B": d2, "3B": d3, "HR": hr, "G": g_f, "P": p_f}])
...
# --- 7. SECCIÓN 💾 RESPALDO ---
    ...
    st.download_button("📥 Descargar Jugadores", df_j.to_csv(index=False), "respaldo_jugadores.csv")
    f = st.file_uploader("📤 Restaurar", type="csv")
    if f: pd.read_csv(f).to_csv(JUGADORES_FILE, index=False); st.success("Restaurado"); st.rerun()

# --- 8. RESTO DE SECCIONES (HISTORIAL, EQUIPOS, ROSTERS) ---
elif menu == "📜 HISTORIAL":
    if not df_j.empty:
        j_s = st.selectbox("Buscar Jugador:", sorted(df_j["Nombre"].unique().tolist()))
        d = df_j[df_j["Nombre"] == j_s].iloc[0]
        st.metric("AVG Carrera", f"{(d['H']/d['VB'] if d['VB']>0 else 0):.3f}")
        st.write(f"**Historial Acumulado:** H: {int(d['H'])} | 2B: {int(d['2B'])} | 3B: {int(d['3B'])} | HR: {int(d['HR'])} | G: {int(d['G'])} | P: {int(d['P'])}")
    ...
    st.write(f"Uniendo familias desde **Agosto de {ANIO_INICIO_LIGA}**.")
